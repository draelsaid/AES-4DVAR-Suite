...
var_qt = sqrt(10);            % True model error variance
...
%-------------------------------------------------------------------------
var_b = 1e-3;               % Background variance
...
nta = obst*obs_f_t; % Assimilation window length, set upto final obs time.
...
% [phi] = test_adv1d_grad(xb,h,ht,a,dt,dx,nt,nx,Binv,Rinv,Qinv,ystore);
...
%% Set initial guess
xo = xb;                     % x^a_0 = x^b_0
eta = zeros(nx+1,nt);        % Error increment eta = 0 initially
z = zeros((nt+1)*(nx+1),1);  % x0 and all eta_i's stored here for gradient solving procedure              

% Arrange etas and xo into one vector, z.
z(1:nx+1) = xo;
z(nx+2:(nx+1)*(nt+1)) = reshape(eta,nx+1,nt);

% Minimisation
xstr='Maximum number of iterations'; ystr='Tolerance'; zstr={'30','1d-5'};
...
afun = @(z) calcJDJ(z,zeros(size(xb)),h,zeros(size(ht)),a,dt,dx,nt,nx,B,R,Q,ystore);
b = - calcJDJ(zeros(size(z)),xb,h,ht,a,dt,dx,nt,nx,B,R,Q,ystore);

Ax = calcJDJ(z,zeros(size(xb)),h,zeros(size(ht)),a,dt,dx,nt,nx,B,R,Q,ystore);
[X_a,FLAG,it] = pcg(afun,b);

%  X_a will contain the solution for the full assimilation window

z = reshape(X_a,nx+1,nt+1);
X_0 = z(:,1);
eta = z(:,2:nt+1);
...
for n=1:nt
xastore(:,n)=xd2;
[X_A]=adv1d(xd2,eta(:,n),nx,a,1,dt,dx);
xd2=X_A;
