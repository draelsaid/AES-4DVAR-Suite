...
xc = L/2; sigma = 1; Ampl = 2; % Mode, Standard dev and Amplitude     
for i=1:nx+1
...
% [Forward_Product,Adjoint_Product,Accuracy] = test_adjoint(a,L,dx,tassim,dt) %
...
for n=1:nt
xastore(:,n)=xd2;
[mx]=adv1d(xd2,nx,a,1,dt,dx);
xd2=mx + eta(:,n);
% figure(10), plot(xs,X_A,'b'), title('Analysis run'), xlabel('Position'), legend('Analysis from t_0 to t_{now}'), ylabel('Value')
% drawnow
...
%% Covariance Matrix Q
figure('Position',pos_5), contour(Q), colorbar, set(gca,'YDir','reverse'), ...
xlabel('Model gridpoint'), ylabel('Model gridpoint'), title('Covariance Matrix Q used in Assimilation')

%% Options print out
min_str{3} = num2str(tassim);
