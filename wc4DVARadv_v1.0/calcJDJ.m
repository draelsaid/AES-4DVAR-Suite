...
Z = reshape(z,nx,nt+1); xo = Z(:,1); eta = Z(:,2:nt+1);

% State trajectory storage of assimilating model
xstore = zeros(nx,nt+1);  
xstore(:,1) = xo;
%-------------------------------------------------------------------------%
%% Calc cost function
J=0;
...
[mx]=adv1d(xstore(:,n-1),a,nx,dx,1,dt);
...
djn = zeros(nx,nt+1);      % Gradient place holder
DJ = zeros(nx*(nt+1),1);   % Gradient
lambda=zeros(nx,(nt+1));   % Adjoint variable
...
djn(:,nt+1) = Jq*[(Qinv*eta(:,nt) - lambda(:,nt+1))];
  ...
  for n=nt:-1:2
   [mlambda]=adv1d_adj(lambda(:,n+1),a,nx,dx,1,dt);
...
% first term of gradient involves background term, not model error term as above
  [mlambda]=adv1d_adj(lambda(:,2),a,nx,dx,1,dt);
