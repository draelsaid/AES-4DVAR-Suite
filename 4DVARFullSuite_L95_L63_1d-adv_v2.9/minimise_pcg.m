function [x,flag,relres,iter,resvec,xmat] = minimise_pcg(A,b,tol,maxit,M1,M2,x0,varargin)
...
%
%   [X,FLAG,RELRES,ITER,RESVEC,XMAT] = PCG(A,B,...) also returns value of X
%   at each minimisation iterate.
%
%   Example:
...
%   $Revision: 1.18.4.12 $ $Date: 2011/05/17 02:33:10 $
...
rel_grad = 1;
while  ii < abs(maxit) && rel_grad >= tol % while not finished and criterion not reached
       ii = ii + (maxit>0);
...
%%%%%
    ...
    rel_grad = norm(resvec(ii+1))/norm(resvec(1))
end
fprintf('\n');
