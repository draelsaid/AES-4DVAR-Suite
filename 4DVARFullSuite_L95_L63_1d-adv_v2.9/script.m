...
% Model settings
N=100; n=30;  % Size of spatial and temporal domains
...
sigmabt = 0.4; Lsbt = 0.03;  % True sigma_b and lengthscale for C_B
sigmaqt = 0.2; Lsqt = 0.02;  % True sigma_q and lengthscale for C_Q
...
% Strong-constraint model propogators
fnlmsc = @(xsc) Mnl(xsc,fnl,schnl,dt,n);
ftlmsc = @(dxsc,xsc) Mtl(dxsc,xsc,ftl,fnl,schtl,schnl,dt,n); 
fadjmsc = @(dysc,ysc) Madj(dysc,ysc,fadj,fnl,schadj,schnl,dt,n);
...
sigmab = 0.4; Lsb = 0.03;   % Estimated sigmab and lengthscale for B
sigmaq = 0.2; Lsq = 0.02;   % Estimated sigmaq and lengthscale for Q
sigmao = 0.1;               % Estimated sigmao
