...
% Model settings
N=50; n=2;  % Size of spatial and temporal domains
dt = 0.01;    % 0.025 dt => 3 hour time step (L95), 1 time unit is 5 days
dx = 1/(2*N);     % Spatial interval size
c=-0.3;
C = (c*dt)/dx

mlvl = 1; % This sets the assimilation step per model level. 
            % modlvl = 2 means the obs are every 2 model steps, 
            % hence the assimilation step is every two model steps.
...
sigmabt = 1; Lsbt = 0.001*uu;  % True sigma_b and lengthscale for C_B
...
obs_f_x = 25;      % Obs frequency (space) (Regular spacing. =1 full obs)
...
sigmab = 0.1; Lsb = 0.001*uu;  % Estimated sigmab and lengthscale for B
sigmaq = 0.05; Lsq = 0.005;  % Estimated sigmaq and lengthscale for Q
sigmao = 0.05;              % Estimated sigmao
...
clear rtD
O = zeros(N,N); [rtD{1:n+1,1:n+1}] = deal(O);
[~,~,Brt,Qrt] = fD(pb);
rtD{1,1} = Brt;
for i=2:n+1
       rtD{i,i} = Qrt;
...
[X_aPdp,FLAGPdp,relresPdp,~,dJXPdp,xmatPdp] = minimise_pcg(wcdpfun,bbdp,tolerance,max_iterations,rtD,rtD,p);
...
% storeitdx(uu) = itdx;
% storeerrdx(uu) = terrordx;

storeitdp(uu) = itdp;
storeerrdp(uu) = terrordp;

storeitPdp(uu) = itPdp;
storeerrPdp(uu) = terrorPdp;

sigmabsigmaq(uu) = sigmab/sigmaq;
sigmabsigmao(uu) = sigmab/sigmao;
sigmaqsigmao(uu) = sigmaq/sigmao;
storehesssize(uu)=(n+1)*N;

[kappaSp,kappaPSp,kappaD] = HessNumCondNoCalc(n,N,fD,fR,fM,obs_f_x,mlvl)

% storekappaSx(uu) = kappaSx;
storekappaSp(uu) = kappaSp;
storekappaPSp(uu) = kappaPSp;
