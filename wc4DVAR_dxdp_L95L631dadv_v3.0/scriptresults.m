...
% Model settings   %CHANGE MODEL SETTINGS FROM INSIDE MODEL%%
N=25; n=100;   % Size of spatial and temporal domains
dt = 0.04;    % 0.025 dt => 3 hour time step (L95), 1 time unit is 5 days
dx = 1/N;     % Spatial interval size
c = -1;
C = (c*dt)/dx

mlvl=1;  % if this is used, don't use obs_f_t
...
sigmabt = 0.5; Lsbt = 0.08;  % True sigma_b and lengthscale for C_B
sigmaqt = 0.3; Lsqt = 0.04;  % True sigma_q and lengthscale for C_Q
...
plot(xs,xatrajdx(:,i),'--r','LineWidth',1.5), plot(xs,xttraj(:,i),'k:','LineWidth',2.5), ...
...
[kappaSx,kappaSp,kappaD] = HessNumCondNoCalc(n,N,fD,fR,fM,obs_f_x,mlvl)
