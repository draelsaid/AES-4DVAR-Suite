...
Lsb = 0.025; Lsq = 0.01;    % Correlation lengthscale
...
%B=sigma_b^2*I;Binv=inv(B); Q=sigma_q^2*I;Qinv=inv(Q); 
...
Sp = Dinv + sigma_o(kk)^(-2)*G;                       % p formulation Hessian
Sx = L'*Dinv*L + sigma_o(kk)^(-2)*H'*H;               % x formulation Hessian

PSp = eye(size(H'*H)) + sigma_o(kk)^(-2)*(rtD*G*rtD); % precond p formulation Hessian
