...
% clearvars -except etat  %% USE THIS TO PRESERVE MODEL ERRORS
% Import errors
...
% Model settings
N=50; n=5;  % Size of spatial and temporal domains
dt = 0.01;    % 0.025 dt => 3 hour time step (L95), 1 time unit is 5 days
dx = 1/(2*N);     % Spatial interval size
c=-1;
C = (c*dt)/dx

%% TRUTH
fnlt = @adv1d; scht = @lpde;    % TRUE Non-lin model and scheme func handles
fM = @() adveqn(c,N,dx,dt);     % Solely for condition number calc.

% IF THE MODEL IS LINEAR, CHECK MINIMISATION PROCEDURE SUITABILITY.

% True Covariance Matrices D and R
sigmabt = 1; Lsbt = 0.05;   % True sigma_b and lengthscale for C_B
sigmaqt = 1; Lsqt = 0.005;  % True sigma_q and lengthscale for C_Q
...
xttraj(:,1) = xto;
for i=1:n
xttraj(:,i+1) = Mnl(xttraj(:,i),fnlt,scht,dt,1) + etat(:,i+1);
end
...
obs_f_x = 10;      % Obs frequency (space) (Regular spacing. =1 full obs)
...
sigmab = 1; Lsb = 0.05;   % Estimated sigmab and lengthscale for B
sigmaq = 1; Lsq = 0.005;  % Estimated sigmaq and lengthscale for Q
...
xstr='Maximum number of iterations'; ystr='Tolerance'; zstr={'2500','1d-10'};
...
Pp = p; 
fDp = @(x) dmtxprecon(x,sigmab,sigmaq,fcB,eye(N));

[bbPdp,~] = wcdp(zeros(size(Pp)),pb,y,fH,fHT,fD,fR,fnlm,fadjm); bbPdp = -bbPdp;
wcPdpfun = @(Pp) wcdp(Pp,pb,y,fH,fHT,fD,fR,fnlm,fadjm) + bbPdp;

[X_aPdp,FLAGPdp,relresPdp,~,dJXPdp,xmatPdp] = minimise_pcg(wcPdpfun,bbPdp,tolerance,max_iterations,fDp,[],Pp);
...
legend('Errors J(p)', 'Errors J(x)'); hold;
...
plot(xs,xttraj(:,i),'k:','LineWidth',2.5), xlabel('Space'), ylabel('Value'), ...
end
...
subplot(2,2,k), hold on, plot(xs,etat(:,i)), plot(xs,eta(:,i),'r'), xlabel('Space'), ...
ylabel('Value'), title(['\eta_{' num2str(i) '}']), ...
end
legend('\eta^t','\eta','Location',[0.86 0.89 0.1 0.1])
...
subplot(2,1,1)
semilogy(linspace(1,max(size(JXdp)),max(size(JXdp))),JXdp,'LineWidth',1.5), title('Convergence of cost function, J(p)'), xlabel('Iteration'), ylabel('Cost function'), axis([1,inf,0,inf]),
subplot(2,1,2)
for i = 1 : itdp
  g =  dJXdp(:,i);
  graddp(i)=norm(g);
end
semilogy(linspace(1,max(size(graddp)),max(size(graddp))),graddp,'LineWidth',1.5), title('Convergence of gradient, J(p)'), xlabel('Iteration'), ylabel('Norm of gradient'), axis([1,inf,0,inf]),

%% Convergence plot J(x)
h3=figure('Position',pos_6); clf; 
subplot(2,1,1)
semilogy(linspace(1,max(size(JXdx)),max(size(JXdx))),JXdx,'r','LineWidth',1.5), title('Convergence of cost function, J(x)'), xlabel('Iteration'), ylabel('Cost function'), axis([1,inf,0,inf]),
subplot(2,1,2)
for i = 1 : itdx
  g =  dJXdx(:,i);
  graddx(i)=norm(g);
end
semilogy(linspace(1,max(size(graddx)),max(size(graddx))),graddx,'r','LineWidth',1.5), title('Convergence of gradient, J(x)'), xlabel('Iteration'), ylabel('Norm of gradient'), axis([1,inf,0,inf]),

%% Convergence plot J(x_0)
h3=figure('Position',pos_6); clf; 
subplot(2,1,1)
semilogy(linspace(1,max(size(JXsc)),max(size(JXsc))),JXsc,'g','LineWidth',1.5), title('Convergence of cost function, J(x_0)'), xlabel('Iteration'), ylabel('Cost function'), axis([1,inf,0,inf]),
subplot(2,1,2)
for i = 1 : itsc
  g =  dJXsc(:,i);
  gradsc(i)=norm(g);
end
semilogy(linspace(1,max(size(gradsc)),max(size(gradsc))),gradsc,'g','LineWidth',1.5), title('Convergence of gradient, J(x_0)'), xlabel('Iteration'), ylabel('Norm of gradient'), axis([1,inf,0,inf]),
...
figure(i+500), hold on, ...
title(['t=' num2str(ts)]), plot(xs,xatrajdp(:,i),'b','LineWidth',1.5), ...
plot(xs,xatrajdx(:,i),'--r','LineWidth',1.5), ... %plot(xs,xatrajsc(:,i),'g','LineWidth',1.5), ...
plot(xs,xttraj(:,i),'k:','LineWidth',2.5), xlabel('Space'), ylabel('Value'), ...
    legend('Solution J(p)','Solution J(x)','Truth','Location',[0.86 0.85 0.1 0.1])
end
...
figure(i+150), hold on, ...
plot(xs,etat(:,i),'LineWidth',1.5), plot(xs,eta(:,i),'r','LineWidth',1.5), xlabel('Space'), ...
ylabel('Value'), title(['\eta_{' num2str(i) '}']), legend('\eta^t','\eta')
end

%[kappaSx,kappaSp,kappaD] = HessNumCondNoCalc(n,N,fD,fR,fM,obs_f_t,obs_f_x)